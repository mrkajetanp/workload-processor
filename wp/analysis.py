...
class WorkloadAnalysisRunner:
    ...
    def trace_pixel6_emeter_df(self, trace):
        return pl.from_pandas(trace.ana.pixel6.df_power_meter().reset_index())

    def trace_energy_estimate_df(self, trace):
    ...
    def trace_frequency_residency_df(self, trace):
        try:
    ...
    def trace_overutilized_df(self, trace):
    ...
    def trace_sched_pelt_cfs_df(self, trace):
        df = pl.from_pandas(trace.df_event("sched_pelt_cfs").reset_index())
        df = df_add_cluster(df)
        return df

    def trace_tasks_residency_time_df(self, trace):
    ...
    def trace_wakeup_latency_cgroup_df(self, trace):
    ...
    def trace_uclamp_df(self, trace):
    ...
    def trace_perf_counters_df(self, trace):
    ...
    def trace_capacity_df(self, trace):
